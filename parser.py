...
print (type(z('ОРГ 1.2')), z('ОРГ 1.2'))
...
def send_welcome(message):
    ...
    users[chat_id] = {}
    bot.send_message(message.chat.id,"Введите фамилию и имя через пробел")
...
def handle_reply_buttons(message):
    if message.text not in answers:
        bot.send_message(message.chat.id, "ты че даун, снова /start сделай пж")
        return
    ...
    if answers[message.text]['new'] == 0:
        ...
        if z(users[message.chat.id]['fulladdress']) == users[message.chat.id]['fulladdress']:
            bot.send_message(message.chat.id, 'Ты по моему перепутал, братанчик. Нет такой группы, иди лучше /start нажми и еще раз попробуй')
            return
...
#добавить кнопки на английски/ирс