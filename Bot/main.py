...
def handle_reply_buttons(message):
    ...
    if answers[text]['new'] == 2:
        ...
        # эта штучка берет данные из словаря и ищет по ним функцию

        func = func_map.get(answers[text]['start'])
        #тут переменная func становится равна функции, которую мы выбрали

        if func:
            raw_data = pars(url)  # парсим таблицу
            ans = func(raw_data, chat_id, users)  # тут мы передаем данные пользователя в переменную, которая стала функцией
        #если мы функцию не нашли, тут уже гг, надо по новой
        else:
            ans = 'Неизвестная категория'

        # если мы не нашли пользователя в группе, то моя функция выдает "ты не в этой группе"
